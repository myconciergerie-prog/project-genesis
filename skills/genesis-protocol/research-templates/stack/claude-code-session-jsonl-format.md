...
sources:
  ...
  - https://github.com/anthropics/claude-code/issues/36583
...
confidence: high
supersedes: ../archive/claude-code-session-jsonl-format_2026-04-15.md
---

# Claude Code Session JSONL Transcript Format — 2026-04-19 Refresh

This entry **supersedes** `claude-code-session-jsonl-format_2026-04-15.md`. Content is refreshed with 2026-04-19 web research; on-disk verification facts from 2026-04-15 (Claude Opus 4.6 session) are preserved verbatim where still accurate.
...
- **Sub-agent sessions** are stored as sibling JSONL files in the same directory, named `agent-<agentId>.jsonl`.
...
**Not observed in this session** but documented in prior art: `summary` (periodic long-session compaction marker), `result` (session completion markers), `git_snapshot` (working tree snapshots). The skill must handle unknown outer types gracefully — classify as `other` and pass through.
...
**Known issue**: on session resume, Claude Code writes `file-history-snapshot` entries whose `messageId` values can collide with existing message `uuid` values in the JSONL file (GitHub issue #36583). The parser must not assume uuid uniqueness across all record types.
...
This entry's TTL is 1 day (`stack/` category). The next refresh should:

1. Verify the slug derivation rule is still `\|:|space|_|/ → -` (Claude Code may change the encoding)
