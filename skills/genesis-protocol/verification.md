...
| 43 | Synthetic partial-citations fixture — only `idea_summary_source_citation` present, others absent. | Step 0.4 card: `Vision` row carries `[page N]` suffix; Project name / Stack hints / Is-a-plugin / extras rows render without suffix. Per-row gate evaluation confirmed: each row inspects its own citation key independently. Step 0.5 `## Fields` Vision row has suffix, others do not. (Reasoning-only probe — no canonical fixture exists for partial-citations; carries the same −0.2 Pain-driven deduction as runtime-replay-deferred scenarios.) |
...
**Ship gate (v1.4.1)**: **#40, #41, #42, #44 mandatory** (happy path / fallback-absence / legacy-config.txt / Mixed media honesty). **Strongly recommended**: #43 (synthetic partial-citations). **Regression on v1.4.0 mandatory**: #37, #38 (Layer B parser zero-ripple on extended schema + fallback byte-identity). **Regression on v1.3.2 mandatory**: #18, #19 (Layer B card + bootstrap_intent.md surface — v1.4.1 extends both templates, legacy behaviour must still render correctly). **Regression on v1.3.1 mandatory**: #7 (Layer A mirror unchanged).

Runtime replay of #40 / #42 driven by a fresh Claude Code process in an empty directory is not executable from inside the ship session. Artefact-level verification is the ship gate — template inspection, grep probes on `<citation>` placeholder counts, fixture round-trip via `Read`, mental walk-through of per-row gate evaluation. A consistent −0.2 Pain-driven deduction applies per replay-deferred scenario, continuing the v1.3.1 → v1.4.0 convention.
...
**Ship gate (v1.4.2)**: **S1, S2, S3 all mandatory** — install-path resolution correctness + R8 cache seed from skill-local + manifest failure-loudness are the three BLOCKER-severity validations that must pass. **Regression on v1.4.1 mandatory**: #40 (Layer B citation render happy path — unchanged, zero Layer B touch in v1.4.2), #41 (absence-of-citations render parity — unchanged). **Regression on v1.3.2 mandatory**: #18, #19 (Layer B card + bootstrap_intent.md surface — v1.4.2 does not touch these templates). **Zero Layer A ripple probe**: `git diff main --stat -- skills/genesis-drop-zone/` must return empty (verified at Task 11.1 pre-commit gate).

**Runtime replay note**: S1 is replayable standalone via a test install path. S2 is replayable as an S1→S2 chain (its Phase 1 prerequisite IS S1). S3 is replayable standalone via `rm skills/genesis-protocol/rules/v1_rules.md` + re-run install-manifest verification. Per the v1.3.1 → v1.4.1 convention, runtime replay deferred if in-session replay is not practical — −0.2 Pain-driven deduction per replay-deferred scenario rolls forward.
