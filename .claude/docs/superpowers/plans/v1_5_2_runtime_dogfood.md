...
---
name: v1.5.2 Runtime dogfood — implementation plan
description: Phase A (autonomous pre-runtime) + Phase B (autonomous post-runtime) + Phase C (ship) + Phase D (conditional Layer 0) + Phase E (chore). Bite-sized tasks with exact paths, commands, expected output. Spec at `.claude/docs/superpowers/specs/v1_5_2_runtime_dogfood.md`. Phase A is fully autonomous — halts at runtime gate where user spawns 5 fresh Claude Code sessions per runbook. Phase B resumes autonomously once evidence pasted.
type: plan
spec: .claude/docs/superpowers/specs/v1_5_2_runtime_dogfood.md
version: v1.5.2
---

# v1.5.2 Runtime Dogfood — Implementation Plan
...
**Architecture:** Layer A-only session. Fixture prep + runbook + evidence log under `skills/genesis-drop-zone/tests/`. New Alexandre fixture at `C:/tmp/genesis-v1.5.2-alexandre/`. Zero Layer B ripple enforced by AC10 diff check. Real Anthropic Messages API subprocess calls on 4 fixtures ; EXIT_NO_KEY halt card runtime on Fixture A. Hybrid blocker gate (class A in-feat / B+C defer).
...
│   ├── specs/v1_5_2_runtime_dogfood.md                    ← already landed (spec + spec-polish commits)
│   └── plans/v1_5_2_runtime_dogfood.md                    ← THIS FILE
├── memory/master.md                                       ← pattern #4 depth-update
└── skills/genesis-drop-zone/
    └── tests/
        ├── runtime_dogfood_v1_5_2.md                      ← runbook (new, reusable)
        └── runtime_dogfood_evidence_v1_5_2.md             ← evidence log (stubs filled Phase A, data filled Phase B)
```

And external to worktree :
```
C:/tmp/genesis-v1.5.2-alexandre/
├── config.txt
...
One `plan-polish(v1.5.2)` commit per v1.6.1 precedent. P3s optional bundle.
...
git log --oneline feat/v1.5.2-runtime-dogfood | grep -c "plan-polish(v1.5.2)"
```
...
**Files:**
- Create: `C:/tmp/genesis-v1.5.2-alexandre/config.txt`
- Create: `C:/tmp/genesis-v1.5.2-alexandre/catalogue_fenetres_fr.md`
- Create: `C:/tmp/genesis-v1.5.2-alexandre/specs_usine_pl.md`
- Create: `C:/tmp/genesis-v1.5.2-alexandre/voice_memo_alexandre.txt`
- Create: `C:/tmp/genesis-v1.5.2-alexandre/photo_facade_client.jpg`

- [ ] **Step 1: Verify `C:/tmp/` exists + create subdir**

```bash
test -d /c/tmp && mkdir -p /c/tmp/genesis-v1.5.2-alexandre
ls -la /c/tmp/genesis-v1.5.2-alexandre
```
...
```bash
python -c "from PIL import Image, ImageDraw; im=Image.new('RGB',(400,300),(200,220,240)); d=ImageDraw.Draw(im); d.rectangle([50,50,350,250],outline=(50,50,50),width=3); d.rectangle([80,80,190,220],fill=(180,200,220)); d.rectangle([210,80,320,220],fill=(180,200,220)); im.save('/c/tmp/genesis-v1.5.2-alexandre/photo_facade_client.jpg','JPEG',quality=85)"
ls -l /c/tmp/genesis-v1.5.2-alexandre/photo_facade_client.jpg
```
...
```bash
ls -l /c/tmp/genesis-v1.5.2-alexandre/
```
...
**Files:**
- Create: `skills/genesis-drop-zone/tests/runtime_dogfood_v1_5_2.md`
...
```bash
grep -c "^##[^#]" skills/genesis-drop-zone/tests/runtime_dogfood_v1_5_2.md
```
Expected : ≥ 4.
...
**Files:**
- Create: `skills/genesis-drop-zone/tests/runtime_dogfood_evidence_v1_5_2.md`
...
```bash
grep -c "^### Fixture" skills/genesis-drop-zone/tests/runtime_dogfood_evidence_v1_5_2.md
```
Expected : `5`.
...
- [ ] **Step 2: Edit `"version": "1.5.1"` → `"version": "1.5.2"`**

Use Edit tool, `old_string="\"version\": \"1.5.1\""`, `new_string="\"version\": \"1.5.2\""`.
...
- [ ] **Step 2: Append sentence after the eighth data-point note, marking v1.5.2 as a depth update on the sixth data-point**
...
git diff --name-only main...HEAD \
  ...
  | grep -vE '^\.claude/docs/superpowers/(specs/v1_5_2_runtime_dogfood\.md|resume/2026-04-19_v1_5_2.*|plans/v1_5_2_runtime_dogfood\.md|research/INDEX\.md)$' \
  | grep -vE '^memory/(project/session_v1_5_2.*|MEMORY\.md|master\.md)$' \
  | grep -v '^CHANGELOG\.md$' \
  | wc -l
```
Expected : `0`.
...
git add .claude/docs/superpowers/plans/v1_5_2_runtime_dogfood.md
git add memory/master.md
git add skills/genesis-drop-zone/tests/runtime_dogfood_v1_5_2.md
git add skills/genesis-drop-zone/tests/runtime_dogfood_evidence_v1_5_2.md
```
...
feat-core(v1.5.2): runtime dogfood prep — runbook + evidence stub + fixture + master.md depth

Phase A pre-runtime commit. Creates :
- skills/genesis-drop-zone/tests/runtime_dogfood_v1_5_2.md (reusable runbook)
- skills/genesis-drop-zone/tests/runtime_dogfood_evidence_v1_5_2.md (evidence stub, 5 fixtures)
- .claude-plugin/plugin.json bump 1.5.1 → 1.5.2
- memory/master.md pattern #4 depth-update on sixth data-point

Alexandre fixture at C:/tmp/genesis-v1.5.2-alexandre/ (5 artefacts, external to repo per R2.5).
...
```bash
git log --oneline feat/v1.5.2-runtime-dogfood | head -4
```
...
Please spawn 5 fresh Claude Code sessions, one per fixture cwd :
  ...
  5. cd C:/tmp/genesis-v1.5.2-alexandre/ && claude

Runbook at :
  skills/genesis-drop-zone/tests/runtime_dogfood_v1_5_2.md
...
```bash
cd "C:/Dev/Claude_cowork/project-genesis/.claude/worktrees/feat_2026-04-19_v1_5_2_runtime_dogfood"
grep -c "TBD at runtime" skills/genesis-drop-zone/tests/runtime_dogfood_evidence_v1_5_2.md
```
...
**Files:**
- Modify: `skills/genesis-drop-zone/tests/runtime_dogfood_evidence_v1_5_2.md`
...
Classify each observed friction into class A/B/C per § 4.3. Class A → record with proposed fix path. Class B+C → record with proposed deferred ship (v1.5.3+).
...
```bash
grep -cE "^\| H[1-5] \| .+ \|" skills/genesis-drop-zone/tests/runtime_dogfood_evidence_v1_5_2.md
```
Expected : ≥ 5.
...
If 3+ class-A with common root cause → ABORT v1.5.2 per § 5 common-root-cause abort. Write partial-evidence CHANGELOG + resume as v1.5.3 unified fix.
If 3+ class-A with distinct root causes → fix first 2 in Task B4, log 3rd+ as "immediate v1.5.3 PATCH chain" in evidence log.
...
Each class-A fix = separate commit `fix-class-a(v1.5.2) #<n>`. Commit message references evidence log line and friction class rationale.
...
```bash
git commit -m "$(cat <<'EOF'
feat-runtime(v1.5.2): evidence filled — H1-H5 per-fixture observations
...
GH_TOKEN="$TOKEN" git push -u origin feat/v1.5.2-runtime-dogfood
```
...
GH_TOKEN="$TOKEN" gh pr create \
  --title "v1.5.2 — runtime dogfood (PATCH)" \
  --body "$(cat <<'EOF'
## Summary
...
- Hybrid blocker gate : class A → in-feat fix, B+C → defer v1.5.3
...
```bash
GH_TOKEN="$TOKEN" gh pr merge --squash --subject "v1.5.2 — runtime dogfood (PATCH)"
```
...
```bash
git tag -a v1.5.2 -m "v1.5.2 — runtime dogfood (PATCH)"
```

- [ ] **Step 2: Push tag**

```bash
GH_TOKEN="$TOKEN" git push origin v1.5.2
```
...
git tag | tail -5
```
Expected chain : `... v1.5.0 v1.5.1 v1.6.0 v1.6.1 v1.5.2`.
...
```bash
grep -c "GH_TOKEN" skills/genesis-drop-zone/tests/runtime_dogfood_evidence_v1_5_2.md
```
...
```bash
touch ~/.claude/memory/layer0/_v1_5_2_layer0_sync_DONE_2026-04-19.md
```
...
```bash
cd C:/Dev/Claude_cowork/project-genesis
git worktree add .claude/worktrees/chore_2026-04-19_v1_5_2_session -b chore/v1.5.2-session main
```

### Task E2: CHANGELOG entry (v1.5.2 honest 5-axis)

**Files:**
- Modify: `.claude/worktrees/chore_2026-04-19_v1_5_2_session/CHANGELOG.md`

- [ ] **Step 1: Add v1.5.2 entry above v1.5.1**
...
**Files:**
- Create: `memory/project/session_v1_5_2_runtime_dogfood.md`
...
Max 150 chars. Points to `session_v1_5_2_runtime_dogfood.md`.

### Task E5: Resume prompt

**Files:**
- Create: `.claude/docs/superpowers/resume/2026-04-19_v1_5_2_to_next.md`

- [ ] **Step 1: Write resume prompt mirroring v1.6.1 structure**

Include : what v1.5.2 shipped, Phase D confirmation checklist, current state, next session candidates (v1.5.3 retirement semantics / v1.6.2 Skill-tool runtime / v1.7.0 new skill), reco + auto-critique, exact phrase for next session, PowerShell launcher one-liner.
...
```bash
cd C:/Dev/Claude_cowork/project-genesis/.claude/worktrees/chore_2026-04-19_v1_5_2_session
git add CHANGELOG.md memory/project/session_v1_5_2_runtime_dogfood.md memory/MEMORY.md .claude/docs/superpowers/resume/2026-04-19_v1_5_2_to_next.md
git commit -m "$(cat <<'EOF'
chore(memory): v1.5.2 — CHANGELOG honest + session trace + MEMORY + resume

Post-ship memory chore for v1.5.2 runtime dogfood.

- CHANGELOG v1.5.2 honest 5-axis rating
- Session trace with hypothesis outcomes + friction triage
- MEMORY.md pointer
- Resume prompt for next session (v1.5.3 / v1.6.2 / v1.7.0 candidates)
...
## Ship-gate checklist (before claiming v1.5.2 done)
...
- [ ] PR opened, squash-merged, tagged v1.5.2
...
Pre-v1.5.2 running avg : 8.90 (18 ratings).
Post-v1.5.2 running avg = (8.90 × 18 + <honest_v1.5.2>) / 19.
