...
## Scope — v1.5.0 Living drop zone memory

### In scope

1. **API requirement — no fallback** — v1.4.0's silent graceful fallback to in-context extraction is **removed**. Genesis v1.5.0 requires `ANTHROPIC_API_KEY` + `anthropic` SDK present. Any failure class (unset key, missing SDK, 401/403, 429 persistent, network unreachable, 5xx transient) produces a bilingual halt-with-remediation card. Rationale: v1.4.0's fallback was preemptive — added without a pain-driven trigger — and became dead weight that v1.5.0 would have to maintain across arbitration paths. Retiring it is anti-Frankenstein rétroactive, triggered by direct user challenge ("pourquoi pas d'API ?").
2. **Divergence detection — two triggers, one arbitration phase** — the extractor prompt is augmented to flag intra-drop divergences (two sources disagree on the same semantic field within the same drop session) via a structured `divergences[]` array in its JSON output. Additionally, a new Phase 0.4 runs in-context (Claude, not Python) to compute cross-session divergences when `drop_zone_intent.md` already exists in cwd and the new extraction differs from the current snapshot. Both trigger types feed the same arbitration phase. Flag-never-resolve principle (KARMA + EMNLP knowledge-conflicts SOTA): LLM detects, human arbitrates; no silent auto-resolution.
3. **Phase 0.5 Arbitration — consolidated bilingual card** — when one or more divergences are detected (intra-drop and/or cross-session), a new arbitration card prints before any write. The card lists each divergent field with its candidate values + their sources + a tag (`[intra-drop]` or `[cross-session]`). Victor tranches in one consolidated response (e.g. `"2,1,2"` or `"autre 3: <value>"`). The selected values replace the extractor's raw output before Phase 0.6 Mirror. Pattern inspired by v1.3.2 consent card (same bilingual shape, same discipline).
4. **Archive pattern — `drop_zone_intent_history/` directory** — when a re-run produces a new snapshot, the existing `drop_zone_intent.md` is moved to `drop_zone_intent_history/v<N>_<ISO8601-Z-timestamp>.md` with additional frontmatter keys added during archival (`status: deprecated`, `archived_at`, `superseded_by: ../drop_zone_intent.md`, `supersession_reason`). The new snapshot is then written with incremented `snapshot_version` and a `supersedes_snapshot: ./drop_zone_intent_history/v<N>_<ts>.md` pointer. Filesystem-based supersession chain, aligned with Layer 0 R8 `archive/` pattern (R8 cache supersession canonical). Genesis's drop zone is now a living memory with revision semantics, not a one-shot-snapshot.
5. **Additive frontmatter keys** — three new optional keys on `drop_zone_intent.md`:
   - `snapshot_version: <int>` — counter starting at 1 on first write, incremented on each supersession
   - `arbitrated_fields: [<field_name>, ...]` — list of field names that went through Phase 0.5 arbitration (empty list `[]` when no arbitration occurred; absent key signals v1.4.x legacy)
   - `supersedes_snapshot: "./drop_zone_intent_history/v<N>_<ts>.md"` — relative pointer to archived predecessor (absent on first write)
   Schema version stays at `1` (additive keys, forward-compat preserved). A v1.4.x Layer B reading a v1.5.0 `drop_zone_intent.md` ignores the three new keys silently (dict-based YAML parse).
6. **Layer B opt-in — `⚖` marker rendering on arbitrated fields** — Phase 0 Step 0.4 intent card and Step 0.5 `bootstrap_intent.md` template render a `⚖` marker suffix on each row whose semantic field name appears in `arbitrated_fields`. Marker positioned after the value + citation + origin tag, before end-of-line. Absent when `arbitrated_fields` is empty or the key is not present (v1.4.x legacy compatibility). Cross-skill-pattern #4 gains a fifth discipline data-point: "Layer B opt-in additive rendering of revision-state metadata".
7. **Halt-with-remediation card — bilingual FR/EN pair** — when API pre-flight fails, a new bilingual halt card prints with: (a) bilingual title "Genesis requires an Anthropic API key" / "Genesis nécessite une clé API Anthropic", (b) remediation text (set env var + relaunch), (c) link to Anthropic console for key creation, (d) note clarifying that Claude Code (Max) subscription does not grant API access. R9 tier 3 bilingual discipline — one paired authoring, EN-top/FR-bottom convention.
8. **Completion vs retirement — asymmetric arbitration triggers** — when comparing `current.<field>` vs `new_extract.<field>` during cross-session detection:
   - **Completion** (`current == null`, `new != null`) = informative log, no arbitration (the new drop adds data that was previously absent — no conflict)
   - **Retirement** (`current != null`, `new == null`) = arbitration required ("keep existing value or accept retirement ?"), because silently retiring a previously-populated field is a destructive act
   - **Divergence** (`current != null`, `new != null`, `current != new`) = arbitration required (the canonical case)
   - **Unchanged** (`current == new`) = no action
9. **Victor-exit safety — no mid-flow state change** — if Victor exits (ctrl+c) or abandons mid-arbitration (Phase 0.5), the existing `drop_zone_intent.md` (if any) remains byte-identical, no archive is created, no new write occurs. Arbitration precedes write; no partial state exists on disk. Next session starts from a clean filesystem state. Anti-data-loss guarantee.
10. **Six-commit rhythm preservation — seventh consecutive application** — (spec + spec polish + plan + plan polish + feat + chore) — same discipline as v1.3.0 through v1.4.1.
11. **Living-spec pattern — seventh consecutive version-scoped section** — this scope section is the seventh application, extending `v2_etape_0_drop_zone.md` with a version-scoped block per ship. Pattern load-bearing status unchanged.

### Out of scope (deferred to v1.5.1+)

- **Layer B expandable archive diff** — Victor hovers / expands the `⚖` marker to see the diff between current and archived value for the arbitrated field. Deferred: v1.5.0 ships the marker as a binary flag; the "show me what changed" UX evolution is v1.5.1+ once dogfood confirms the binary flag is insufficient.
- **Retry policy on 5xx transient errors** — v1.5.0 halts on any 5xx; v1.5.1+ adds auto-retry with exponential backoff (1-2 attempts) before halting. Pain-driven once a concrete transient-error-halt friction is observed.
- **Archive retention policy** — v1.5.0 keeps all archived snapshots indefinitely. v1.5.1+ adds a retention cap (e.g. keep-last-N per Kurrent SOTA, or TTL) if dogfood surfaces disk pressure. Per SOTA consensus (Kurrent, AWS / DynamoDB patterns), keep-last-N is the canonical choice; defer until pain observed.
- **Concurrent re-runs protection** — lock file on `drop_zone_intent.md` during extraction to prevent two parallel Claude Code sessions racing. v1.5.0 has no lock; Victor running two sessions in the same cwd is not a documented use case.
- **Import of an archived snapshot** — Victor wants to re-promote an old snapshot as current. Requires a `/genesis-drop-zone/restore <snapshot>` subcommand or equivalent. Deferred pain-driven.
- **Cross-project memory flow** — facts from project-A inherited by project-B with supersession semantics. Meta-Memory Path B territory; requires universal infrastructure (transverse pointer propagation with consent); out of v1.5.0 scope.
- **Event log alongside snapshot** — the SaaS-grade pattern (Shopify / Stripe / Uber per Layer 0 `pattern_state_machine_outbox_from_day_one.md`). SOTA (Fowler, Kurrent) recommends domain-modeling-first; Genesis's low-frequency single-writer profile does not justify the additional surface. Defer to v2.x.x if multi-writer pain emerges.
- **`cited_text_preview` inline surfacing / hyperlink citations / Files API / programmatic handoff / GH_BROWSER / UX toolkit / chime / bilingual Layer B null-class parsing / three-locale expansion / Structured Outputs Path B pivot** — all unchanged from v1.4.2 deferred list, now renumbered as v1.5.1+.

### Rationale for v1.5.0 route

- **MINOR bump — structural weight from two pains addressed end-to-end** — v1.5.0 addresses intra-drop contradictions (silent LLM resolution was the default) + temporal revision (halt-on-existing was the default) with a unified arbitration phase. Each alone would be a PATCH; together they open a new surface (Phase 0.5 Arbitration, archive directory, new bilingual pair for halt, new bilingual pair for arbitration, `⚖` marker rendering) that reshapes the drop zone flow. The MINOR bump is honest.
- **Fallback removed proactively on user challenge** — v1.4.0's graceful fallback was preemptive (no user hit the "no API" case in real dogfood). v1.5.0 retires it based on direct user challenge and accepts the honest contract: Genesis requires an Anthropic API key + subscription, surfaced via halt-with-remediation rather than silent degradation. Anti-Frankenstein rétroactive.
- **Pain-driven authenticity restored** — intra-drop contradictions are a documented silent-failure mode; temporal revision has no existing UX beyond `halt-on-existing`. Both pains are real and structural. Breaks the v1.4.1 loop-closure pattern; restores pain-driven authenticity.
- **Snapshot-only, not event log** — SOTA 2026 consensus: event logs for multi-writer SaaS; snapshot-only + archive sufficient for low-frequency single-writer memory. Reference R8 entry: `.claude/docs/superpowers/research/sota/living-memory-and-supersession-patterns_2026-04-18.md` § Finding 1.
- **Flag-never-resolve principle** — KARMA + EMNLP knowledge-conflicts SOTA converge: LLM detects contradiction reliably, cannot arbitrate reliably (training-prior bias + recency effects). Extractor prompt flags, Victor arbitrates. Reference § Finding 3.
- **Structural divergence flag over scalar confidence threshold** — Cleanlab TLM + Sensible patterns recommend binary structural signals over LLM-self-reported confidence scores (LLMs empirically overconfident). v1.5.0 uses binary flag (divergence detected / not). Reference § Finding 4.
- **Unitemporal single-time, not bitemporal** — XTDB / Datomic bitemporal overkill for Genesis's single time axis. Reference § Finding 5.
- **Filesystem archive, not cryptographic chain** — activity-timeline pattern canonical; cryptographic tamper-resistance unnecessary for single-writer trust model. Reference § Finding 6.
- **Schema additive, no version bump** — v1.4.0 additive precedent preserved. `schema_version: 1` stays. Cross-skill-pattern #4 fifth data-point: Layer B opt-in additive rendering of revision-state metadata.
- **Pain-driven axis projection 9.0-9.1** — vs v1.4.1's 8.5-8.7 loop-closure honest floor. Two structural pains addressed; axis breaks through 9.0. Running-average lift estimable.
...
| Scope — v1.5.0 Living drop zone memory | `## Scope / In scope (v1.5.0)` sub-block (in/out bullets, copied verbatim) + `## Living memory dispatch (v1.5.0)` (API-required pre-flight + divergence detection + arbitration phase + archive flow + schema additive keys) | Mirrored |
...
| Deferred to v1.5.1+ | `## Deferred scope` (verbatim bullet list, updated at v1.5.0 since v1.5.0 ships Layer A changes; `v1.4.2+` label sweep from v1.4.1 batched into this v1.5.0 touch, plus new items 4-8 + 18-19 reflecting v1.5.0-opened scope) | Mirrored |
...
| Rationale for v1.5.0 route | — | **Spec-only** (design decision log) |
...
| Living memory rendering (v1.5.0) — Step 0.2a recognition of additive revision keys | `### Step 0.2a — Parse drop_zone_intent.md` — new subsection `#### Revision metadata preservation (v1.5.0)` listing the three additive keys (`snapshot_version`, `arbitrated_fields`, `supersedes_snapshot`). Parser mechanics unchanged — documentation-only preservation list | Mirrored |
| Living memory rendering (v1.5.0) — Step 0.4 card `⚖` marker on arbitrated fields | `### Step 0.4 — Surface the parsed intent card` — template extended with `⚖` marker placeholder on rows whose field name appears in `arbitrated_fields`. Absent marker when `arbitrated_fields` is empty or absent | Mirrored |
| Living memory rendering (v1.5.0) — Step 0.5 bootstrap_intent.md `⚖` marker on arbitrated field rows | `### Step 0.5 — Persist the intent as bootstrap_intent.md` — `## Fields` and `## Conversational context from drop zone` tables Value columns gain `⚖` marker suffix on arbitrated field rows | Mirrored |
| Living memory rendering (v1.5.0) — verification scenarios | `skills/genesis-protocol/verification.md` new v1.5.0 scenario block (#45-#49 or contiguous after latest) | Mirrored |
...
**v1.5.0 additions to the tier table** (additive to the v1.4.0 rows above):

| Artefact | Tier | Language |
|---|---|---|
| Phase 0.5 Arbitration card — runtime string template (divergent-field rows + candidate values + source tags + reply-format instruction) | User-facing runtime | **Bilingual FR + EN paired authoring day 1** per R9 tier 3 convention. Canonical EN-top / FR-bottom layout mirroring v1.3.2 consent card discipline. Candidate values echo source language verbatim; tags `[intra-drop]` / `[cross-session]` are language-neutral ASCII. |
| Halt-with-remediation card — runtime string template (API-missing title + remediation instruction + console URL + Claude-Code-subscription note) | User-facing runtime | **Bilingual FR + EN paired authoring day 1** per R9 tier 3 convention. Canonical EN-top / FR-bottom layout. URL is language-neutral. |
| `drop_zone_intent.md` frontmatter `snapshot_version` / `arbitrated_fields` / `supersedes_snapshot` keys | Dev-layer data contract | English keys (snake_case); values language-neutral (integer, array of snake_case field names, relative filesystem path). |
| `drop_zone_intent_history/v<N>_<ts>.md` archive frontmatter extensions (`status: deprecated`, `archived_at`, `superseded_by`, `supersession_reason`) | Dev-layer audit metadata | English keys + language-neutral values (ISO-8601 timestamp, filesystem path, English enum `{'cross-session re-run','intra-drop','manual'}`). |
| `⚖` marker on arbitrated fields in Phase 0 Step 0.4 card + Step 0.5 `bootstrap_intent.md` template | User-facing runtime | Language-neutral ASCII (Unicode balance scale symbol U+2696). No FR/EN branching. Rendering identical under any locale. |

The two new bilingual pairs (arbitration card + halt-with-remediation card) are the only R9 tier-3 surface growth for v1.5.0. The `⚖` marker is deliberately ASCII-symbolic (not a word "arbitrated" / "arbitré" that would require a bilingual pair) so it works uniformly across all locales.

## Deferred to v1.5.1+

Ordered by rough priority, non-binding, revisit at each session boundary. Items closed across the v1.3.x / v1.4.x / v1.5.0 cycle so far: the `bootstrap_intent.md` file write and Layer B handoff (both closed in v1.3.2); **runtime locale detection** (closed in v1.3.3 — welcome + mirror + consent card + halt + bridges + body prose + body mirror echo all dispatch on `welcome_locale` / `content_locale`); **Path A Citations API extraction** (closed in v1.4.0 — second privilege class with graceful fallback, `source_citation` frontmatter extension, mirror annotations); **Layer B citation surfacing** (closed in v1.4.1 — Step 0.4 card + Step 0.5 template render `[page N]` / `[lines X-Y]` suffix inline on mapped + extras rows, zero parser change, zero schema bump, zero Layer A ripple); **Contradictions surfacing / intra-drop arbitration + cross-session revision / living memory archive + supersession + halt-with-remediation on API-missing / Layer B `⚖` marker rendering** (all closed in v1.5.0 — unified arbitration phase covering intra-drop divergences AND cross-session re-run, filesystem archive chain `drop_zone_intent_history/`, fallback path retired in favour of explicit API requirement, three additive frontmatter keys, fifth cross-skill-pattern #4 discipline data-point). This list reflects what remains.

1. **`cited_text_preview` inline surfacing** — the citation dict carries up to 80 chars of quoted source text; v1.4.1 renders only the position annotation. Surfacing the preview inline (e.g. on hover or via `cited_text:` tooltip syntax) is a v1.5.1+ UX option. Preview is already archived verbatim in `drop_zone_intent.md` for any reader who wants the full quote.
...
4. **Layer B expandable archive diff** — Victor hovers / expands the v1.5.0 `⚖` marker to see the diff between current and archived value for the arbitrated field. v1.5.0 ships the marker as a binary flag; the "show me what changed" UX evolution is v1.5.1+ once dogfood confirms the binary flag is insufficient.
5. **Retry policy on 5xx transient errors** — v1.5.0 halts on any 5xx via halt-with-remediation; v1.5.1+ adds auto-retry with exponential backoff (1-2 attempts) before halting. Pain-driven once a concrete transient-error-halt friction is observed.
6. **Archive retention policy** — v1.5.0 keeps all archived snapshots indefinitely. v1.5.1+ adds a retention cap (e.g. keep-last-N per Kurrent SOTA, or TTL) if dogfood surfaces disk pressure.
7. **Concurrent re-runs protection** — lock file on `drop_zone_intent.md` during extraction to prevent two parallel Claude Code sessions racing in the same cwd. v1.5.0 has no lock; documented use case does not exist yet.
8. **Import / restore of an archived snapshot** — Victor wants to re-promote an old snapshot as current. Requires a `/genesis-drop-zone/restore <snapshot>` subcommand or equivalent. v1.5.1+ pain-driven.
9. **Programmatic handoff** — auto-invoke `genesis-protocol` without the user typing `/genesis-protocol` after the accept bridge. Requires a harness-level skill-to-skill invocation mechanism that is not 2026-04 Claude Code ready. Human-in-the-loop via the accept bridge's instruction is the v1.3.2 pattern; the programmatic path can ship when the harness supports it without changing the bridge text semantics.
10. `GH_BROWSER` profile routing wire-up — read Chrome profile map from Layer 0, export `GH_BROWSER` before any `gh` invocation in the downstream LAYER B.
11. UX toolkit integration — `@clack/prompts` structural skeleton, Charm Gum for select prompts, cli-spinners for the `◐` animation. With R9 closed (v1.3.3), citations shipped + surfaced (v1.4.0 + v1.4.1), and living-memory arbitration shipped (v1.5.0), the surface is complete — polish can land without re-fragmenting it.
12. Completion chime (cross-platform) — macOS `afplay`, Windows `[console]::beep`, Linux `paplay`. Honours the "rising interval" convention per vision doc § "The sound of Genesis".
13. Filesystem-side error handling refinements — permission-denied / disk-full / symlink-pointing-to-directory edge cases on the Layer A write path currently let `OSError` bubble up naturally. v1.5.0 ships API-path halt-with-remediation but not filesystem-side equivalents. v1.5.1+ adds filesystem halt + remediation if any of these produces real user pain. v1.5.0's floor is "filesystem write succeeds or harness shows the stack".
14. **Chain-of-Verification (CoVe) second pass** — Haiku 4.5 verification of Citations output. R8 ranks Citations above CoVe and recommends skipping unless evals show failure. Skip until evals demand.
15. **Bilingual Layer B null-class parsing** — if `drop_zone_intent.md` frontmatter null-class tokens ever carry EN canonical variants alongside FR canonical, Layer B's Step 0.2a parser grows a bilingual branch. v1.5.1+ target.
16. **Three-locale-or-more expansion** — if Genesis ships beyond FR + EN (e.g. ES, DE), `welcome_locale` and `content_locale` become n-way enums. Minor, deferred until a real non-FR/EN user emerges.
17. **Structured Outputs (Path B) alternative** — v2 architectural pivot question. Would require dropping Citations (API incompatibility). Not considered until Path A evals expose a concrete shortfall.
18. **Cross-project memory flow** — facts from project-A inherited by project-B with supersession semantics. Meta-Memory Path B territory; requires universal infrastructure (transverse pointer propagation with consent). v2.x.x scope.
19. **Event log alongside snapshot** — the SaaS-grade pattern (Shopify / Stripe / Uber per Layer 0 `pattern_state_machine_outbox_from_day_one.md`). SOTA recommends domain-modeling-first; Genesis's low-frequency single-writer profile does not justify the additional surface yet. Defer to v2.x.x if multi-writer pain emerges.
...
- `living-memory-and-supersession-patterns_2026-04-18.md` (v1.5.0 primary source) — seven SOTA findings backing v1.5.0 design: snapshot-only defensible for low-frequency single-writer (Fowler, Kurrent, domain-modeling-first); LLM flag-never-resolve multi-document conflicts (KARMA paper + EMNLP knowledge-conflicts survey); structural signals over scalar confidence (Cleanlab TLM benchmark + Sensible confidence signals); unitemporal sufficient for single-time memory (not bitemporal XTDB/Datomic); activity-timeline audit UX canonical (MongoDB + Hubifi + Docsie); YAML frontmatter + filesystem archive canonical doc-version pattern (GitHub Docs, dyff). Cross-reference Layer 0 `pattern_state_machine_outbox_from_day_one.md` for the SaaS-grade event-log counterpart (deliberately NOT adopted for Genesis — wrong domain profile). Expires 2026-04-25.
...
The scenarios below cover **seven ship-gate blocks** (v1.3.0 / v1.3.1 / v1.3.2 / v1.3.3 / v1.4.0 / v1.4.1 / v1.5.0). Range map:
...
- Scenarios **#28-#39** — v1.4.0 additions targeting API-path happy cases, four fallback triggers, fallback byte-identity, R9 audit of the Python extractor, Layer B parser zero-ripple on the extended frontmatter, annotation-truncation edge case. **Fallback-path scenarios (#29, #32, #33, #38) become regression probes for v1.5.0 forward-compat (v1.4.x readers of v1.5.0 files) but are no longer exercised at the ship-gate level — v1.5.0 retires the fallback path.**
...
- Scenarios **#45-#59** — v1.5.0 additions: 8 happy-path scenarios (Layer A fresh-bootstrap baseline, intra-drop arbitration, cross-session arbitration, cross-session no-diff skip, mixed intra-drop + cross-session, completion case, retirement case, many-divergences warn) + 5 error-path scenarios (API-missing halt, API-401 halt, malformed-existing-frontmatter halt, malformed-arbitration-response re-prompt, Victor-exit mid-arbitration safety) + 2 forward-compat regression scenarios (v1.4.x `drop_zone_intent.md` re-run creates v2 snapshot, v1.5.0 `drop_zone_intent.md` read by v1.4.x parser ignores new keys). Plus Layer B-side 5 scenarios (`⚖` marker rendering present / absent / unknown-field-defensive / v1.4.x legacy-rendering / `supersedes_snapshot` preservation).
...
### v1.4.1 regression set for v1.5.0

All v1.3.0 + v1.3.1 + v1.3.2 + v1.3.3 + v1.4.0 + v1.4.1 regression guarantees preserved. v1.5.0 modifies three surfaces that intersect with earlier scenarios: (a) the extraction contract (v1.4.0 scenarios #28-#39 fallback-path exercise becomes forward-compat regression only — v1.5.0 retires the fallback), (b) the halt-on-existing path (v1.3.2 scenario #15 becomes superseded by v1.5.0's cross-session arbitration), (c) Layer B rendering (v1.4.1 scenarios #40-#44 continue to apply to the citation part, with v1.5.0 scenarios layering the `⚖` marker orthogonally). Scenarios explicitly superseded are marked inline; other regression probes remain valid.

### v1.5.0 new scenarios — Layer A (`genesis-drop-zone`)

| # | Scenario | Expected |
|---|---|---|
| 45 | Fresh bootstrap, `ANTHROPIC_API_KEY` + SDK present, single document drop, LLM finds no divergence. | API pre-flight passes. Extraction subprocess emits `divergences: []`. Phase 0.4 detection: no intra-drop divergences; no existing `drop_zone_intent.md` (fresh cwd) → no cross-session. Phase 0.5 arbitration skipped. Mirror renders normally (v1.3.3/v1.4.0 parity). Consent + write. Written `drop_zone_intent.md` carries `snapshot_version: 1`, `arbitrated_fields: []`, no `supersedes_snapshot`. No `drop_zone_intent_history/` directory created (no archive needed on first write). |
| 46 | Fresh bootstrap, API + SDK present, 2 documents with intra-drop divergence on `budget` and `stack_hints` fields. | Extraction subprocess emits `divergences: [{field: "budget_ou_contrainte", candidates: [...]}, {field: "hints_techniques", candidates: [...]}]`. Main output leaves those fields null. Phase 0.4 collects 2 intra-drop divergences. Phase 0.5 arbitration card prints bilingual (EN-top / FR-bottom) with 2 rows tagged `[intra-drop]` + candidates + sources. Victor replies `"2,1"` (picks candidate 2 for budget, candidate 1 for stack). Selected values replace nulls in the extractor output. Mirror renders with selected values. Consent + write. Written snapshot: `snapshot_version: 1`, `arbitrated_fields: ["budget_ou_contrainte", "hints_techniques"]`. |
| 47 | Re-run: existing `drop_zone_intent.md` from a prior session (v1.5.0 format, `snapshot_version: 1`), zero diff vs new extraction. | Phase 0.4 detects existing file, compares frontmatter field-by-field, emits empty cross-session divergence list. Phase 0.5 arbitration skipped (combined divergence list is empty). Mirror + consent + write. Archive created: existing file moved to `drop_zone_intent_history/v1_<iso8601-ts>.md` with frontmatter gaining `status: deprecated`, `archived_at`, `superseded_by: ../drop_zone_intent.md`, `supersession_reason: "cross-session re-run"`. New `drop_zone_intent.md` written with `snapshot_version: 2`, `arbitrated_fields: []`, `supersedes_snapshot: "./drop_zone_intent_history/v1_<iso8601-ts>.md"`. Note: this case happens when Victor re-drops the same content — no arbitration needed, but snapshot is still bumped for traceability. |
| 48 | Re-run: existing `drop_zone_intent.md` + new brief that differs on `budget` field only. | Phase 0.4 collects 1 cross-session divergence (`budget_ou_contrainte`). Phase 0.5 arbitration card prints with 1 row tagged `[cross-session — changed since snapshot_version N]`. Victor replies `"2"` (picks the newer value). Archive + write pattern identical to #47. New `drop_zone_intent.md` carries `snapshot_version: 2`, `arbitrated_fields: ["budget_ou_contrainte"]`, `supersedes_snapshot` pointer. |
| 49 | Re-run + intra-drop simultaneously — existing file from v1 + new drop has 2 docs with conflict on `stack` + new drop also differs from v1 on `vision`. | Phase 0.4 collects 1 intra-drop divergence (`stack`) + 1 cross-session divergence (`vision`). Phase 0.5 arbitration card mixes the two: row 1 `[intra-drop]`, row 2 `[cross-session]`. Victor tranches both in one response. Selected values flow forward. Archive + write. `arbitrated_fields: ["stack", "vision"]`. |
| 50 | Completion case — existing file has `budget: null`, new extraction finds `budget: 15000`. | Phase 0.4 detects field change but classifies as completion (null → non-null), NOT divergence. No arbitration entry for this field. Informative stderr log: `"Field completed: budget_ou_contrainte (null → 15000)"`. Other fields evaluated independently. Archive + write. `arbitrated_fields` excludes `budget_ou_contrainte` (completion is not arbitration). |
| 51 | Retirement case — existing file has `stack_hints: "Next.js + Supabase"`, new extraction finds `stack_hints: null` (brief updated and no longer mentions stack). | Phase 0.4 classifies as retirement (non-null → null). Arbitration required. Phase 0.5 card shows the row tagged `[cross-session — retirement]` with two candidates: `[1] keep existing: "Next.js + Supabase"` and `[2] accept retirement: null`. Victor picks. Archive + write. If Victor picks `[2]`, `arbitrated_fields` includes `stack_hints` and the new snapshot has `stack_hints: null`. |
| 52 | Many-divergences case — 12 divergences across intra-drop + cross-session combined. | Extraction + diff compile 12 entries. Stderr warn: `"Many divergences (12). This may indicate contradictory sources or a significantly revised brief."`. Phase 0.5 arbitration card prints all 12 rows (no pagination in v1.5.0). Victor answers with 12 comma-separated choices. Archive + write. No throttle, no cap. |
| 53 | Error — `ANTHROPIC_API_KEY` unset. | Phase 0.2 API pre-flight fails. Halt-with-remediation card prints (bilingual, EN-top/FR-bottom) with title "Genesis requires an Anthropic API key" / "Genesis nécessite une clé API Anthropic", remediation text (set env var + relaunch), console URL, Claude Code subscription note. Zero write. Existing `drop_zone_intent.md` (if any) remains byte-identical. Skill exits clean. |
| 54 | Error — API key set but API returns 401 (invalid key). | Pre-flight submits a lightweight validation request, 401 returned. Same halt-with-remediation card as #53 with an additional sub-line: "Key invalid — check console / Clé invalide — vérifie la console". Zero write. |
| 55 | Error — existing `drop_zone_intent.md` has malformed frontmatter (e.g. invalid YAML). | Phase 0.4 attempts frontmatter parse, fails. Halt card: "Existing drop_zone_intent.md has malformed frontmatter. Fix or move it manually / drop_zone_intent.md existant a un frontmatter corrompu. Corrige ou déplace-le manuellement". Zero write. Existing file preserved byte-identical. |
| 56 | Error — Victor replies arbitration with wrong number of choices (e.g. 3 divergences, answer is `"1,2"` only). | Phase 0.5 parser detects mismatch. Re-prompts with hint: "Expected 3 choices, got 2. Please answer again / Attendu 3 choix, reçu 2. Réessaie". Card re-displayed. Victor corrects. Normal flow resumes. |
| 57 | Error — Victor exits (ctrl+c) mid-arbitration. | Skill exits immediately. Zero write. Zero archive. Existing `drop_zone_intent.md` (if any) byte-identical. No `drop_zone_intent_history/` created if it did not exist prior. Next session starts from clean filesystem state. |
| 58 | Forward-compat — v1.4.x `drop_zone_intent.md` present (no `snapshot_version`, no `arbitrated_fields`, no `supersedes_snapshot` keys). Re-run with new drop. | Phase 0.4 parses frontmatter; absent keys treated as `snapshot_version: 1` implicit, `arbitrated_fields: []` implicit, `supersedes_snapshot: null` implicit. Cross-session diff runs normally. On write: existing v1.4.x file moved to archive with a `supersession_reason: "cross-session re-run (from v1.4.x snapshot)"`. New file is v1.5.0 format with `snapshot_version: 2`. First upgrade across the v1.4.x → v1.5.0 boundary preserves semantics. |
| 59 | Forward-compat — v1.5.0 `drop_zone_intent.md` read by a v1.4.x parser simulation (or any dict-based YAML parser). | All v1.4.x mandatory keys are present. Three v1.5.0 additive keys (`snapshot_version`, `arbitrated_fields`, `supersedes_snapshot`) are silently ignored. Rendering on the v1.4.x parser (if forward-replayed) proceeds identically to v1.4.x / v1.4.1 behaviour. Zero crash. Confirms cross-skill-pattern #4 fifth data-point forward-compat property. |

### v1.5.0 new scenarios — Layer B (`genesis-protocol`)

| # | Scenario | Expected |
|---|---|---|
| 60 | Seed via fixture `drop_zone_intent_fixture_v1_5_0_arbitrated_two_fields.md` with `arbitrated_fields: ["budget_ou_contrainte", "hints_techniques"]`. Run `/genesis-protocol`. | Step 0.2a parses `arbitrated_fields`. Step 0.4 card renders `⚖` marker on the two rows matching the field names (e.g. `Budget / constraint : 15 000 € [page 8] ⚖` and `Stack hints : Next.js + Supabase [page 5] ⚖`). Other rows (Vision, Project name, etc.) have no `⚖`. Step 0.5 `bootstrap_intent.md` template renders the same markers inline in the `Value` columns. |
| 61 | Seed via fixture with `arbitrated_fields: []`. | Step 0.4 card + Step 0.5 template render identically to v1.4.1 — zero `⚖` marker. Empty array is semantically equivalent to absent. |
| 62 | Seed via fixture with `supersedes_snapshot: "./drop_zone_intent_history/v1_..."`. | Step 0.2a parser preserves the key. Step 0.5 `bootstrap_intent.md` template echoes the pointer in its frontmatter (preservation, no rendering). Claude in Phase 0 has silent visibility of the supersession chain but does not render it to Victor on the card in v1.5.0 (deferred to v1.5.1+ expandable-archive-diff UX). |
| 63 | Seed via v1.4.x fixture (no v1.5.0 keys at all). | Step 0.2a parser ignores absent keys silently. Card + template render identically to v1.4.1 baseline. Forward-compat regression pass. |
| 64 | Seed via fixture with `arbitrated_fields: ["obsolete_field_name"]` (unknown field name, not one of the 9 semantic fields). | Layer B ignores the unknown field. No `⚖` marker on any card row (the 9 known rows all have known field names). No crash. Defensive parser behaviour preserved per v1.3.2 dict-based YAML convention. |
...
- **v1.4.1 (previous)**: **#40, #41, #42, #44 mandatory** — Layer B citation render happy path (#40), absence-of-citations render parity on fallback fixture (#41), legacy `config.txt` render parity (#42), Mixed media unadorned honesty (#44). **Strongly recommended**: #43 (synthetic partial-citations). **Regression on v1.4.0 mandatory**: #37 (Layer B parser zero-ripple — unchanged; v1.4.1 extends rendering, not parsing), #38 (fallback byte-identity — unchanged; v1.4.1 does not write anything). **Regression on v1.3.2 mandatory**: #18, #19 (Layer B card + bootstrap_intent.md surface — v1.4.1 extends both templates, so the v1.3.2 behaviour must still render correctly for legacy paths). **Regression on v1.3.1 mandatory**: #7 (Layer A mirror unchanged).
- **v1.5.0 (this ship)**: Layer A — **#45, #46, #48, #53, #57, #58 mandatory** — fresh happy-path baseline (#45), intra-drop arbitration (#46), cross-session arbitration (#48), API-missing halt (#53), Victor-exit safety (#57), v1.4.x forward-compat (#58). Layer B — **#60, #61, #63 mandatory** — `⚖` marker rendering (#60), no-marker regression on empty/absent (#61), v1.4.x legacy forward-compat (#63). **Strongly recommended**: Layer A #47 (re-run no-diff), #49 (mixed divergence triggers), #50 (completion case), #51 (retirement case), #54 (API-401 halt), #55 (malformed-frontmatter halt), #56 (wrong-choice-count re-prompt), #59 (v1.5.0 read by v1.4.x parser); Layer B #62 (supersedes_snapshot preservation), #64 (unknown field defensive). **Documented non-blocking**: Layer A #52 (many-divergences warn — synthetic). **Regression on v1.4.1 mandatory**: #37 (Layer B parser zero-ripple on extended schema — v1.5.0 adds three more additive keys), #40 (Layer B citation render — still applies, v1.5.0 layers `⚖` orthogonally over `[page N]`). **Regression on v1.3.2 mandatory**: #18, #19 (Layer B surface + precedence rule — v1.5.0 extends templates again, both must still work for legacy paths). **Regression on v1.3.1 mandatory**: #7 (Layer A mirror unchanged structurally — the mirror content changes when arbitration triggers, but the mirror template + render loop are structural invariants). **Explicitly superseded**: v1.4.0 #29, #32, #33, #38 (fallback-path scenarios — v1.5.0 retires the fallback; these scenarios become v1.4.x historical probes only, not executed at ship gate); v1.3.2 #15 (halt-on-existing behaviour — v1.5.0 replaces halt-on-existing with cross-session arbitration as the default re-run UX; the legacy halt behaviour can still be recovered via R2.1 worktree discipline on Victor's side but is not the default flow).

**Runtime replay note — extended for v1.5.0**: runtime replay of scenarios driven by a fresh Claude Code process in an empty directory (including v1.4.0's #28-#35 which exercise live API + subprocess launch, v1.4.1's #40-#42 which exercise live Phase 0 card render, and v1.5.0's #45 #46 #48 #53 #57 Layer A + #60 #61 #63 Layer B scenarios which exercise live arbitration card + halt card + marker rendering) is not executable from inside a running session. Artefact-level verification remains the ship gate — Python module importability (`python -c "import skills.genesis-drop-zone.scripts.extract_with_citations"`), SKILL.md + phase-0-seed-loading.md dispatch-logic walkthrough, fixture byte-identity via `diff`, JSON / YAML parse validation of fixtures, Layer B parser regression on the extended fixture, grep assertions on the rendered card template (e.g. `[page N]` presence / absence counts, `⚖` marker presence / absence counts). A consistent −0.2 Pain-driven deduction applies per replay-deferred scenario and rolls forward until runtime replay happens, continuing the v1.3.1 → v1.4.1 convention into v1.5.0.
...
- **MINOR bump — two structural pains + surface reshape justifies the semver weight** — v1.5.0 addresses intra-drop contradictions (silent LLM resolution was the default) AND temporal revision (halt-on-existing was the default) with a unified arbitration phase. These are not refinements to existing surfaces — they open a NEW phase (0.5 Arbitration) in the drop zone flow, a NEW archive directory (`drop_zone_intent_history/`), two NEW bilingual string templates (arbitration card + halt-with-remediation), a NEW Layer B rendering discipline (`⚖` marker on arbitrated fields). Each alone would deserve a PATCH within a generous semver reading; together they reshape the flow and deserve MINOR. Running average 8.89 has 0.39 tampon above 8.5 — MINOR with ≥ 9.0 self-rating remains within the streak envelope.
- **Fallback removal triggered by user challenge, not engineering speculation** — v1.4.0's silent graceful fallback to in-context extraction was added preemptively, covering six failure classes (unset key, missing SDK, 401/403, 429, bad input, invalid output). It was never pain-driven validated: no Victor hit the "no API key" case in real dogfood across v1.4.0 → v1.4.1. The user's direct question mid-brainstorm ("pourquoi pas d'API ?") surfaced the fallback as a Frankenstein-lite preemption that v1.5.0 would have had to maintain across the arbitration phase (intra-drop detection in fallback path would require either prompt augmentation of in-context Claude with no structured-output guarantee, or degraded silent resolution that contradicts the "flag-never-resolve" principle). Retiring the fallback is anti-Frankenstein rétroactive — and the cleaner surface justifies the halt-with-remediation card as the honest contract ("Genesis requires an Anthropic API key + subscription").
- **Pain-driven axis authenticity restored** — v1.4.0 was pain-driven MINOR (audit trail); v1.4.1 was honest loop-closure PATCH (Pain-driven 8.5–8.7 because no user pain triggered the ship, only system-asymmetry did); v1.5.0 returns to pain-driven MINOR. Two documented pain vectors:
  - **Silent intra-drop resolution** — LLMs are empirically overconfident (Cleanlab TLM bench + arxiv 2306.13063 + openreview consensus); auto-resolving contradictions without Victor's input hides a real data-integrity problem and produces a drop_zone_intent.md that can mislead Phases 1-7 downstream.
  - **Halt-on-existing as the only re-run UX** — v1.3.2's halt was a safety feature (prevent overwrite) that becomes friction when Victor legitimately returns 2 weeks later with a revised brief. No UX existed to let him update the file without manual intervention. v1.5.0's cross-session arbitration is the missing flow.
- **Snapshot-only + archive directory, not event log** — SOTA 2026 consensus (R8 `living-memory-and-supersession-patterns_2026-04-18.md` § Finding 1) is explicit: event logs are the canonical audit pattern for multi-writer SaaS marketplaces (Shopify / Stripe / Uber / Airbnb per Layer 0 `pattern_state_machine_outbox_from_day_one.md`). Snapshot-only + archive is sufficient for low-frequency single-writer memory (Fowler / Kurrent / AWS Prescriptive). Genesis's drop zone = single-user, single-project, low-frequency → snapshot-only is the right choice. Adopting event sourcing would be anti-Frankenstein (domain-modeling-first discipline says the simpler pattern suffices). If Genesis ever grows a multi-writer use case (team bootstrap, organizational memory), event log becomes a v2.x.x evolution — the archive directory is the same filesystem shape, easy to extend.
- **Flag-never-resolve principle from LLM multi-document conflict research** — KARMA paper (OpenReview 2026) + EMNLP 2024 knowledge-conflicts survey converge on: LLM detects contradiction reliably via multi-agent architectures with Conflict Resolution Agents, but arbitration is delegated to human-in-the-loop (LLM-proposed resolution biased by training-prior recency + authority heuristics that don't generalize). v1.5.0's extractor prompt augmentation implements this: `divergences[]` array flags the conflict, main output leaves the field null, human arbitration fills it. No silent auto-resolve.
- **Structural divergence flag, not scalar confidence threshold** — Cleanlab TLM benchmark (2026): trust scores beat LLM-self-reported confidence by 25% precision/recall on structured outputs — but the benchmark itself proves that scalar confidence is unreliable. Sensible's "confidence signals" (binary structural flags: multiple-answers, partial, uncertain, no-answer) is the alternative pattern. v1.5.0 uses the binary structural flag: divergence present OR absent. No configurable threshold ("confidence > 0.8 = skip arbitration") because it would re-introduce the unreliable scalar path.
- **Unitemporal single-time snapshot, not bitemporal** — XTDB / Datomic bitemporal (system-time + valid-time) is essential for audit-regulated systems ("what did you know and when did you know it?"). Genesis has ONE time axis — when the snapshot was written. No distinct "valid-time" concept (Victor doesn't care that the brief was revised at 2026-04-15 vs we extracted it at 2026-04-18; what matters is the extraction timestamp). Adopting bitemporal would add dimensions without justifying pain. R8 § Finding 5.
- **Filesystem-based supersession chain, not cryptographic chaining** — Activity-timeline audit UX (MongoDB document versioning, Hubifi audit trail 2026, Docsie 2026) is the canonical pattern. Cryptographic chaining (each entry hash-tied to previous, tamper detection) is required for compliance-grade systems with third-party auditors. Genesis's trust model is single-writer single-machine; Victor does not need to prove to himself that his own snapshots were not tampered with. Filesystem + ISO-8601 timestamp naming is sufficient. R8 § Finding 6.
- **Consolidated arbitration card, not per-field prompts** — brainstorm explored two alternatives: (a) per-field prompts (Genesis asks Victor one divergence at a time, sequential), (b) consolidated card (all divergences in one card, single Victor response). Per-field was rejected because it introduces N consecutive prompts = Victor fatigue with no value gain (the 9 semantic fields are independent; arbitrating one doesn't inform another). Consolidated matches the v1.3.2 consent card pattern — same bilingual shape, same discipline, one new paired authoring. The UX scales without paginated views (scenario #52 "many divergences" validates even at N=12).
- **`⚖` marker as Layer B opt-in, not deferred** — user explicitly said "sur la card ça doit être traité rapidement" during brainstorm Section 2 Q4 → the marker ships in v1.5.0, not v1.5.1 PATCH. Justification: the marker is the visible signal that a field carries revision history. Without it, Victor arbitrates + sees values in the card but has no indication that arbitration happened — the audit trail is invisible at the rendering layer. Shipping the marker in v1.5.0 ensures that end-to-end revision visibility is complete at MINOR ship, not an afterthought.
- **Unicode `⚖` over text marker** — explored three candidates: (a) text suffix `[arbitré]`/`[arbitrated]` (R9 bilingual pair), (b) colon-label `arbitrated:` English only, (c) Unicode symbol `⚖` (balance scale, U+2696). Option (a) requires a bilingual pair and clutters the card; option (b) is English-only and contradicts R9 tier-3 for a user-facing surface; option (c) is language-neutral, compact, semantically meaningful (balance scale = trade-off weighed). Selected. R9 tier-3 discipline preserved without a new bilingual pair.
- **Schema additive preserved — no version bump — zero Layer A ripple for v1.4.x readers** — v1.5.0 follows the v1.4.0 additive precedent. `schema_version: 1` stays. Three new optional keys (`snapshot_version`, `arbitrated_fields`, `supersedes_snapshot`). v1.4.x Layer B parsers ignore the new keys silently (dict-based YAML parse). Forward-compat property verified via scenarios #58-#59. Cross-skill-pattern #4 fifth data-point: "Layer B opt-in additive rendering of revision-state metadata" (v1.3.2 wire + v1.3.3 body-vs-frontmatter asymmetry + v1.4.0 additive keys + v1.4.1 additive rendering of citations + v1.5.0 additive rendering of revision state).
- **Archive retention policy deferred to v1.5.1+** — Kurrent SOTA recommends keep-last-N. v1.5.0 ships indefinite retention. Retention policy = v1.5.1+ pain-driven (if dogfood surfaces disk pressure). Honest narrow scope; avoids adding a config surface without observed need.
- **Activity-timeline UX (expandable archive diff on marker) deferred to v1.5.1+** — v1.5.0 ships the marker as binary flag, not the "show me what changed" expansion. Progressive disclosure UX pattern applied: simplest useful surface first, richer interactions pain-driven. If dogfood shows Victor wanting diff visibility at the marker, v1.5.1+ PATCH adds it.
- **Six-commit rhythm seventh consecutive application** — (spec + spec polish + plan + plan polish + feat + chore). Pattern load-bearing status unchanged across the v1.3.x → v1.4.x → v1.5.0 transitions.
- **Living-spec seventh consecutive version-scoped section** — extending `v2_etape_0_drop_zone.md` with `## Scope — v1.5.0 Living drop zone memory` preserves the vein-of-truth pattern. Seven data-points (v1.3.0 / v1.3.1 / v1.3.2 / v1.3.3 / v1.4.0 / v1.4.1 / v1.5.0) make the pattern full load-bearing. A new reader walks the seven scope sections to see what layered on, then reads the seven rationale H2 sections for the "why" of each ship.
- **Pain-driven axis projection 9.0-9.1** — vs v1.4.1's 8.5-8.7 loop-closure honest floor. Two structural pains addressed (intra-drop contradictions + temporal revision). Self-rating projection: Pain-driven 9.0-9.1, Prose cleanliness 9.1-9.3 (large surface but single narrative), Best-at-date 9.1-9.3 (fresh R8 `living-memory-and-supersession-patterns_2026-04-18.md` entry grounds 12 design choices), Self-contained 8.8-9.0 (large surface honestly assumed — 2 skills touched, 15 new scenarios Layer A + 5 new Layer B, 8 new fixtures estimate), Anti-Frankenstein 9.3-9.4 (fallback removal rétroactive + flag-never-resolve + snapshot-only over event log). Average ≈ 9.10-9.15.
- **Ship size honest — 8-12h effective session in autonomous mode** — this is the largest v1.x ship since v1.4.0 (which introduced the citation path). The MINOR weight is honestly reflected in the session budget. Spec already covers the full scope; plan task breakdown will concretize the ~12-14 implementation tasks.
